...
// Accept incoming client connections and relay published messages to all
wss.on('connection', (socket, req) => {
	socket.on('message', (data) => {
		// Broadcast received messages to all other clients (detector -> UI)
		try {
			const txt = typeof data === 'string' ? data : data.toString();
			// optional: validate JSON
			JSON.parse(txt);
			wss.clients.forEach(c => {
				try { if (c.readyState === 1) c.send(txt); } catch (e) {}
			});
